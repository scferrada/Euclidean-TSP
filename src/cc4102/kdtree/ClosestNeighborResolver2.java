...
public class ClosestNeighborResolver2 implements EuclideanTSPResolver{
        ...
        while (porVisitar.size()>0){
            ...
            for(int i: visitedPoints){
                for(int j: porVisitar){
                    int ind;
                    if(j<i){
                        ind = (((2*points.size()*j)+j-(j*j))/2) + (i-j);
                    }
                    else{
                        ind = (((2*points.size()*i)+i-(i*i))/2) + (j-i);
                    }


                    double candmin= dm[ind];
                    if(candmin<min){
                        min=candmin;
                        indexmnin=j;
                    }
